---
title: "Flurex Microbiome Import and QC"
...
knit: (function(inputFile, encoding) { 
    rmarkdown::render(
        ...
        output_file = paste0("Flurex_", Sys.Date(), "_ImportQC.html")) 
    })
params:
    input: "input/flurex_data_full.RData"
    meta: "input/metadata_flurex_full.csv"
    neg: "NEGATIVE|WTR"
    batch: "run"
...
This document contains the commands necessary to perform initial import and QC of the output data from the DF_GMH_PIPELINE (DTU FOOD, Group for Gut, Microbes and Health) and merging with metadata. Here it has been fitted for data from Flurex (internal project name: R20-22). The project data contains 16S V3 amplicon sequencing from feces samples from 4 different days and cecum and ileum samples from one day from 48 rats. There are feces samples from all rats from day 0, 2, 4 and 8. Cecum and ileum samples were all collected at dissection on day 8. Please read the method section for this paper for details on experimental setup.
...
if (!file.exists("plots/decontam")) dir.create(file.path(getwd(), "plots/decontam"))
if (!file.exists("plots/mock")) dir.create(file.path(getwd(), "plots/mock"))
...
sample_data(phy)$ID <- with(sample_data(phy), paste(Seq_run,Sample, sep = "_"))
...
It can be easier to use the build in import function to import correctly and then save the created code here to reproduce import later (File > Import Dataset > From Text (readr)).
...
meta <- read.csv(params$meta, header = T, sep = ";")

# Create an identical ID variable to use for merging
meta$ID <- with(meta, paste(run,name, sep = "_"))
...
suppressMessages(ggsave("plots/decontam/sequencing_depth.pdf"))
...
sample_data(phy)$quant_reading <- ifelse(sample_data(phy)$dnaC == 0, min(sample_data(phy)$dnaC[sample_data(phy)$dnaC != 0])/2, sample_data(phy)$dnaC)
...
suppressMessages(ggsave(decontam.plot,file = "plots/decontam/contaminant_fraction_multiple.png",device = "png"))
suppressMessages(ggsave(decontam.plot,file = "plots/decontam/contaminant_fraction_multiple.pdf",device = "pdf"))
...
![Abundance classified as contaminant](plots/decontam/contaminant_fraction_multiple.png){width="100%"}

```{r Decontam_filter, eval=FALSE, echo=TRUE}
...
# Evaluate what you can agree to loose and then use that column. I will use the minimum.fisher
phy <- prune_taxa(contam.df$ASV[contam.df$minimum.fisher == FALSE], phy)
...
df$Index <- seq(nrow(df))
plot_seqd_norm <- ggplot(data=df, aes(x=Index, y=depth, color=type)) + geom_point() + 
  facet_wrap(params$batch, nrow = 1) + ggtitle("Sequencing depth after Decontam")
suppressMessages(ggsave(plot_seqd_norm,file = "plots/decontam/sequencing_depth_after_decontam.pdf",device = "pdf"))
...
df <- df[order(df$depth),]
df$Index <- seq(nrow(df))
plot_seqd_harsh <- ggplot(data=df, aes(x=Index, y=depth, color=type)) + geom_point() + 
  facet_wrap(params$batch, nrow = 1) + ggtitle("Sequencing depth after harsh Decontam")
suppressMessages(ggsave(plot_seqd_harsh,file = "plots/decontam/sequencing_depth_after_harsh_decontam.pdf",device = "pdf"))
...
suppressMessages(ggsave("plots/mock/test_mock_comparison.png",mock.plot,device = "png"))
...
![Mock community comparison](plots/mock/test_mock_comparison.png){width="100%"}
...
COL <- c("CTRL" = "#606060","PFOS" = "#31b44b","VAN" = "#6699d1","VAN+PFOS" = "#fcdd05")
...
plot.ind <- ggplot(Rdat.m, aes_string(x = "depth", y = "Alpha_diversity", color = params$batch)) + 
  ...
  geom_vline(color = "red",xintercept = 15000) +  xlim(0,60000) +
  theme_pubclean() + scale_color_brewer(palette = "Paired")
suppressMessages(ggsave(filename = "plots/adiv/Rarefaction/Rcurve_individual.png",plot = plot.ind, device = "png", units = "mm", width = 200, height = 100, dpi = 300))
suppressMessages(ggsave(filename = "plots/adiv/Rarefaction/Rcurve_individual.pdf",plot = plot.ind, device = "pdf", units = "mm", width = 200, height = 100, dpi = 300))
...
plot.group <- ggplot(Rdat.m, aes(x = depth, y = Alpha_diversity, color = Batch_type)) + 
  ...
  geom_vline(color = "red",xintercept = 15000) +  xlim(0,60000) +
  theme_pubclean() + scale_color_brewer(palette = "Paired")
suppressMessages(ggsave("plots/adiv/Rarefaction/Rcurve_grouped.png", plot = plot.group, device = "png", units = "mm", width = 200, height = 100, dpi = 300))
suppressMessages(ggsave("plots/adiv/Rarefaction/Rcurve_grouped.pdf", plot = plot.group, device = "pdf", units = "mm", width = 200, height = 100, dpi = 300))

# Create plot for treatment groups for day 0 and remaining days (feces samples)
# Subset
load("R_objects/Rare_dat.RData")
MTRL <- "Feces"
DAY <- "d0"
Rdat.m <- Rdat.m[Rdat.m$material %in% MTRL & Rdat.m$day %in% DAY,]

# plot per treatment group
plot.treat <- ggplot(Rdat.m, aes(x = depth, y = Alpha_diversity, color = treatment)) + 
  geom_smooth(aes(group = treatment), method = "loess", formula = y ~ x, se = FALSE) + 
  facet_wrap("Index", scales = "free",nrow = 1) + 
  geom_vline(color = "red",xintercept = 15000) +  xlim(0,60000) +
  theme_pubclean() + scale_color_manual(values = COL)
suppressMessages(ggsave(paste0("plots/adiv/Rarefaction/Rcurve_treatment_",MTRL,"_",DAY,".png"), plot = plot.treat, device = "png", units = "mm", width = 200, height = 100, dpi = 300))
suppressMessages(ggsave(paste0("plots/adiv/Rarefaction/Rcurve_treatment_",MTRL,"_",DAY,".pdf"), plot = plot.treat, device = "pdf", units = "mm", width = 200, height = 100, dpi = 300))
...
plot.treat <- ggplot(Rdat.m, aes(x = depth, y = Alpha_diversity, color = treatment)) + 
  ...
  geom_vline(color = "red",xintercept = 15000) +  xlim(0,60000) +
  theme_pubclean() + scale_color_manual(values = COL)
suppressMessages(ggsave(paste0("plots/adiv/Rarefaction/Rcurve_treatment_",MTRL,"_day2-8.png"), plot = plot.treat, device = "png", units = "mm", width = 200, height = 100, dpi = 300))
suppressMessages(ggsave(paste0("plots/adiv/Rarefaction/Rcurve_treatment_",MTRL,"_day2-8.pdf"), plot = plot.treat, device = "pdf", units = "mm", width = 200, height = 100, dpi = 300))
...
![Rarefaction_curves_individual](plots/adiv/Rarefaction/Rcurve_individual.png){width="100%"} 
Rarefaction curve grouped by sample type and batch: 
![Rarefaction_curves_grouped](plots/adiv/Rarefaction/Rcurve_grouped.png){width="100%"}
Rarefaction curve grouped by sample type and batch: 
![Rarefaction_curves_grouped](plots/adiv/Rarefaction/Rcurve_treatment_Feces_d0.png){width="100%"}
Rarefaction curve grouped by sample type and batch: 
![Rarefaction_curves_grouped](plots/adiv/Rarefaction/Rcurve_treatment_Feces_day2-8.png){width="100%"}
...
plot.ind <- ggplot(Rdat.m, aes_string(x = "depth", y = "Alpha_diversity", color = params$batch)) + 
  geom_smooth(aes(group = Sample), se = FALSE) + 
  facet_wrap("Index", scales = "free",nrow = 1) + 
  geom_vline(color = "red",xintercept = 11000) + xlim(0,60000) +
  theme_pubclean() + scale_color_brewer(palette = "Paired")
suppressMessages(ggsave(filename = "plots/adiv/Rarefaction/Rcurve_individual_harsh.png",plot = plot.ind, device = "png", units = "mm", width = 200, height = 100, dpi = 300))
suppressMessages(ggsave(filename = "plots/adiv/Rarefaction/Rcurve_individual_harsh.pdf",plot = plot.ind, device = "pdf", units = "mm", width = 200, height = 100, dpi = 300))
...
plot.group <- ggplot(Rdat.m, aes(x = depth, y = Alpha_diversity, color = Batch_type)) + 
  ...
  geom_vline(color = "red",xintercept = 11000) + xlim(0,60000) +
  theme_pubclean() + scale_color_brewer(palette = "Paired")
suppressMessages(ggsave("plots/adiv/Rarefaction/Rcurve_grouped_harsh.png", plot = plot.group, device = "png", units = "mm", width = 200, height = 100, dpi = 300))
suppressMessages(ggsave("plots/adiv/Rarefaction/Rcurve_grouped_harsh.pdf", plot = plot.group, device = "pdf", units = "mm", width = 200, height = 100, dpi = 300))
...
plot.treat <- ggplot(Rdat.m, aes(x = depth, y = Alpha_diversity, color = treatment)) + 
  ...
  geom_vline(color = "red",xintercept = 11000) +  xlim(0,60000) +
  theme_pubclean() + scale_color_manual(values = COL)
suppressMessages(ggsave(paste0("plots/adiv/Rarefaction/Rcurve_treatment_",MTRL,"_",DAY,"_harsh.png"), plot = plot.treat, device = "png", units = "mm", width = 200, height = 100, dpi = 300))
suppressMessages(ggsave(paste0("plots/adiv/Rarefaction/Rcurve_treatment_",MTRL,"_",DAY,"_harsh.pdf"), plot = plot.treat, device = "pdf", units = "mm", width = 200, height = 100, dpi = 300))
...
plot.treat <- ggplot(Rdat.m, aes(x = depth, y = Alpha_diversity, color = treatment)) + 
  ...
  geom_vline(color = "red",xintercept = 11000) +  xlim(0,60000) +
  theme_pubclean() + scale_color_manual(values = COL)
suppressMessages(ggsave(paste0("plots/adiv/Rarefaction/Rcurve_treatment_",MTRL,"_day2-8_harsh.png"), plot = plot.treat, device = "png", units = "mm", width = 200, height = 100, dpi = 300))
suppressMessages(ggsave(paste0("plots/adiv/Rarefaction/Rcurve_treatment_",MTRL,"_day2-8_harsh.pdf"), plot = plot.treat, device = "pdf", units = "mm", width = 200, height = 100, dpi = 300))
...
Rarefaction curve for individual samples:
![Rarefaction_curves_individual](plots/adiv/Rarefaction/Rcurve_individual_harsh.png){width="100%"} 
Rarefaction curve grouped by sample type and batch:
![Rarefaction_curves_grouped](plots/adiv/Rarefaction/Rcurve_grouped_harsh.png){width="100%"}
Rarefaction curve for individual samples:
![Rarefaction_curves_individual](plots/adiv/Rarefaction/Rcurve_treatment_Feces_d0_harsh.png){width="100%"} 
Rarefaction curve for individual samples:
![Rarefaction_curves_individual](plots/adiv/Rarefaction/Rcurve_treatment_Feces_day2-8_harsh.png){width="100%"} 
...
phy <- prune_samples(sample_sums(phy) > 15000, phy)
...
save(phy, file="R_objects/Phyloseq_15000.Rdata")

# remove low read samples and mock from harshly decontaminated
phy <- prune_samples(sample_sums(phy.harsh) > 11000, phy.harsh)
phy <- subset_samples(phy, type == "Sample")
phy <- prune_taxa(taxa_sums(phy) > 0, phy)
phy_tree(phy) <- midpoint(phy_tree(phy))

# save harshely decontaminated samples
save(phy, file="R_objects/Phyloseq_harsh_11000.Rdata")
