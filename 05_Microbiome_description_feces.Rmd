---
title: "Flourex Microbiome description"
...
knit: (function(inputFile, encoding) { 
  rmarkdown::render(
      ...
      output_file = paste0("Flurex_", Sys.Date(), "_Description.html")) 
    })
params:
...
params <- readRDS( "R_objects/params_description")
...
# print tax table
# print tax table
tax <- dat.sort %>% distinct(Taxa, .keep_all = T)%>% select(Taxa, Kingdom, Phylum)
...
# print tax table
# print tax table
tax <- dat.sort %>% distinct(Taxa,.keep_all = T) %>% select(Taxa, Kingdom, Phylum, Class)
...
# print tax table
# print tax table
tax <- dat.sort %>% distinct(Taxa,.keep_all = T) %>% select(Taxa, Kingdom, Phylum, Class, Order)
...
# print tax table
# print tax table
tax <- dat.sort %>% distinct(Taxa,.keep_all = T) %>% select(Taxa, Kingdom, Phylum, Class, Order,Family)
...
suppressMessages(ggsave(filename = "plots/relabu/relative_abundance_plot_all_genus.png", plot = p, device = "png", dpi = 300, units = "mm", height = 200, width = 300))
suppressMessages(ggsave(filename = "plots/relabu/relative_abundance_plot_all_genus.pdf", plot = p, device = "pdf", dpi = 300, units = "mm", height = 200, width = 300))
# print tax table
# print tax table
tax <- dat.sort %>% distinct(Taxa,.keep_all = T) %>% select(Taxa, Kingdom, Phylum, Class, Order,Family,Genus)

try(kable(tax,digits = 2,caption = paste("Taxa plotted", sep = " ")) %>%
  kable_classic(full_width = F, position = "left") %>%
  save_kable(file = "plots/relabu/relative_abundance_plot_all_genus_complist.pdf"))
...
# print tax table
# print tax table
tax <- dat.sort %>% distinct(Taxa,.keep_all = T) %>% select(Taxa, Kingdom, Phylum, Class, Order,Family,Genus,Species)
...
|------------|-----------------------|-------------------------------------|
| Statistical testing    | GMH_test_variables         | Statistical test of sample_data variables, incl alpha diversity |
| Beta diversity         | GMH_betadiversity          | Statistical test of beta diversity and visualization            |
